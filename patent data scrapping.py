...
def get_patent_data(patent):
    ...
    data=[patent, title.string, inventors, assignee.text, current_assignee.text.strip(), prd.text, flgdate.text, pubdate.text]
    return data


df=pd.DataFrame(index=['Patent Number', 'Title', 'Inventor', 'Assignee', 'Current Assignee', 'Priority Date', 'Application Date', 'Publication Date'])
